...
**Tools** are functions that agents can call to take actions.



extend their capabilities beyond pure language reasoning. Tools bridge the gap between what a model "knows" and what it can "do"—enabling agents to perform actions like getting the current time, querying APIs, searching the web, or executing custom business logic.

When you provide tools to an agent, the model can decide when to use them based on the conversation context. The agent automatically handles tool invocation, processes results, and incorporates them into its responses.

---

## Python Functions as Tools

The simplest way to create tools is to wrap Python functions. Any function can become a tool that your agent can call.
...
**How it works:**
- The `Tool()` wrapper converts your function into a tool specification
- Docstrings become tool descriptions that help the model understand when to use the tool
- Type hints define parameter types (converted to JSON schema)
- Both sync and async functions are supported
- The agent automatically decides when to call your tools

---

## MCP Tools

**Model Context Protocol (MCP)** enables agents to use tools from external servers. MCP servers provide standardized interfaces to various services—web search, databases, APIs, and more.
...
Agents automatically have access to several built-in tools:
...
Enable interactive conversations where agents can pause and request user input. See [Human-in-the-loop](/agents/human-in-the-loop) for complete documentation on the `ask_user_for_input` tool.
...
**Tool Factories** enable you to create scope-aware tools that are instantiated per-user, per-conversation, or per-agent. This is essential for multi-tenant applications where tools need isolated resources.
...
```python
from autonomy import Agent, Model, Node, Tool
from autonomy.tools import ToolFactory
...
**How it works:**
1. You pass the factory instance to `tools=[db_factory]`
2. The agent framework detects it implements `ToolFactory`
3. For each conversation, the framework calls `create_tools(scope, conversation, agent_name)`
4. You create tools with proper isolation based on the provided context
5. Each user/conversation gets their own tool instances with isolated resources
...
The framework automatically coerces JSON values to match Python type hints:
...
The agent can see the error and decide how to respond—retry with different parameters, ask for clarification, or explain the problem to the user.
...
- **[Human-in-the-loop](/agents/human-in-the-loop)** - Enable agents to pause and request user input
- **[Filesystem Tools](/agents/filesystem)** - Complete filesystem operations with configurable visibility
- **[Subagents](/agents/subagents)** - Hierarchical agent coordination and delegation
- **[Knowledge](/agents/knowledge)** - Search document collections with KnowledgeTool
- **[Context](/agents/context)** - How tool specifications are included in context
- **[Memory](/agents/memory)** - How tool calls and results are stored
