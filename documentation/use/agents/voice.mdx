---
title: "Voice"
description: "Build real-time voice agents with low-latency audio interaction."
icon: "microphone"
---

Voice agents enable real-time audio conversations with your users. They use a
**Voice Interface pattern** where a fast voice model handles immediate user
interaction while delegating complex tasks to a more powerful primary agent.

---

## Architecture

The Voice Interface pattern uses two agents working together:

1. **Voice Agent** - Uses OpenAI's Realtime API for low-latency voice interaction.
   Handles greetings, chitchat, and simple clarifications directly.

2. **Primary Agent** - Your main agent with tools and powerful reasoning.
   Handles complex questions, database lookups, and tool-based tasks.
