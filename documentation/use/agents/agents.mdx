...
<img src="/agents/images/agent.png"
  alt="An agent is a state machine that calls an LLM and tools, in a loop." />
...
Agents can invoke [python functions as tools](/tools#python-functions-as-tools),
or call tools provided by [MCP servers](/tools#mcp-tools). They can access a
...
the [built-in HTTP API](/programming-interfaces#built-in-apis).
...
For longer responses, use the [streaming API](/programming-interfaces#built-in-apis)
