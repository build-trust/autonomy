...
Claude Cowork runs in a VZVirtualMachine (Apple's Virtualization framework). To use Docker inside Claude Cowork, you need to expose your Docker daemon to the macOS VM.

If you use Docker Desktop, enable "Expose daemon on tcp://localhost:2375 without TLS" in Docker Desktop settings, then create an SSH tunnel to expose it to the VM that Claude Cowork runs in.

If you use Podman as your Docker daemon on macOS, you can expose it to Claude Cowork by setting up an SSH tunnel:

**On your Mac host (run this before starting Claude Cowork):**
...
<CardGroup cols={2}>
  <Card title="Agents" icon="star-shooting" iconType="solid" href="/agents/agents">
