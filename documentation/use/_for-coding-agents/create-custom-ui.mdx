...
**Use pnpm over npm**: pnpm is faster, more efficient with disk space, and provides better dependency resolution. Install pnpm if not already available:

```bash  theme={null}
npm install -g pnpm
```

**Configure pnpm for security**: Set up pnpm to only install packages that have been available for at least 7 days to avoid supply chain attacks:

```bash  theme={null}
# Create or update .npmrc in your project root
echo "minimum-release-age=10080" >> .npmrc
```

This setting ensures that only packages released 7+ days ago (10080 minutes) are installed, providing time for the community to identify and report malicious packages.
...
```json  theme={null}
{
  "scripts": {
    ...
    "build-autonomy": "pnpm run build && rm -rf ../images/main/public/* && cp -r [output-dir]/* ../images/main/public/",
...
pnpm run build
pnpm run build-autonomy
```

**Best Practices:**
- Always configure `minimum-release-age=10080` in `.npmrc` to avoid newly released malicious packages
- Run `pnpm audit --fix` before every deployment
- Keep dependencies updated with `pnpm update --latest`
- Test your application after dependency updates
- Use `pnpm` instead of `npm` for better performance and reliability
