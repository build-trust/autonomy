...
# A guide for coding agents to create and deploy custom APIs for Autonomy apps and agents
...
*Keywords: fastapi, custom api, parallel agents, http server, endpoints*

This guide shows how to create custom FastAPI routes and build scalable multi-agent architectures.
...
The `index.html` would have to be adapted to call the `/translate` endpoint. To start, we can test using curl.
...
The `autonomy.yaml` and `Dockerfile` remain the same as in the [getting started guide](https://autonomy.computer/docs/_for-coding-agents/create-a-new-autonomy-app.md). The `index.html` would have to be adapted to call the `/translate` endpoint. To start, we can test using curl.
...
- [How to create custom user interfaces](https://autonomy.computer/docs/_for-coding-agents/create-custom-ui.md)
