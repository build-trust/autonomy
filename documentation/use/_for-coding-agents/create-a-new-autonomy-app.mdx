...
Create an encrypted portal to the logs server in your zone. This will show
streaming logs to the app.

```bash
# Create a portal to the logs server in background and capture output to extract its port number
autonomy zone inlet --to logs > portal_to_logs_server.log 2>&1 &
LOGS_PID=$!

# Wait for the portal to be created and extract its port number
sleep 3
LOGS_PORT=$(grep -o "localhost:[0-9]*" portal_to_logs_server.log | cut -d: -f2 | head -1)
if [ -z "$LOGS_PORT" ]; then
    LOGS_PORT=32101  # fallback to default port
fi

echo "Portal to the logs server has an inlet on port: $LOGS_PORT"
echo "Portal to the logs server PID: $LOGS_PID"
```
