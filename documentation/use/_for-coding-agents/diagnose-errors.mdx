...
❌ **WRONG** (too short for cold start):
...
**Test Strategy**: Node discovery → worker creation → messaging → full processing.
...
- Test progressively: discovery → workers → messaging → full system
...
5. **Progressive testing?** Simple → complex
