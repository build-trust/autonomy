...
description: "Build an agentic app that uses Linkup search APIs as tools to research new user signups and provide personalized outreach recommendations."
---
...
signup does not scale. An AI agent with [Linkup](https://linkup.so) search APIs as tools can do this automatically.
...
The agent has two tools to gather information using the Linkup API:
...
Research a new signup and stream the results via `POST /api/research`.
