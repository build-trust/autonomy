...
Edit `secrets.yaml` with your 1Password Connect token:

```yaml secrets.yaml
OP_CONNECT_TOKEN: "your_1password_connect_token_here"
```

Credentials use the standard 1Password reference format: `op://vault/item/field`
</Step>
<Step title="Deploy">
...
Provide a brief assessment of the {agent_type} health for this service.""",
  ...
  responses = await agent.send(f"Diagnose {agent_type} health for {service_name} in {region}.")
  return {"agent_type": agent_type, "status": "completed", "finding": responses[-1].content.text}
```
...
diagnosis = await synthesis_agent.send(f"Synthesize findings: {findings_summary}")
```
