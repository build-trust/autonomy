...
**Option 1: View logs in browser**
```bash
# Open logs in your web browser (shows streaming logs with nice UI)
open http://127.0.0.1:$LOGS_PORT
```

**Option 2: Stream logs in terminal**
```bash
# Get the dynamic pod name and stream logs directly
POD_NAME=$(curl -s http://127.0.0.1:$LOGS_PORT/ | grep -o 'main-pod-[a-z0-9]*-[a-z0-9]*' | head -1)
curl -s http://127.0.0.1:$LOGS_PORT/pods/$POD_NAME/containers/main
```
