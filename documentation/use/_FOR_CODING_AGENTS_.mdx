...
mode: "wide"
description: "This file has instructions for Claude, Codex, Copilot, Cursor, Devin, Cline, Replit, Zed Agent and other coding agents to create and run apps on the Autonomy Computer."
---
...
**Important:** For interactive UIs, use the streaming API endpoint (`/agents/agent-name?stream=true`) instead of the regular endpoint for real-time responses.
...
If you do need to generate a new UI, prefer generating streaming output for better user experience with real-time responses.

## Streaming Implementation

When creating interactive web applications, implement streaming for better UX:

```javascript
// Use ?stream=true and process with response.body.getReader()
const response = await fetch('/agents/your-agent?stream=true', {
    method: 'POST',
...
const decoder = new TextDecoder();
// Parse JSON chunks line by line and display incrementally
```

Consider adding character-by-character display using a text queue and setTimeout for smooth typewriter effects.
...
At this point the new zone's directory should look like this:
...
The `autonomy` command uses docker to build docker images and push them to a Zone in a Cluster in the Autonomy Computer.
</Step>
...
# Streaming API (preferred for UIs)
curl --request POST \
--header "Content-Type: application/json" \
--data '{"message":"who are you?"}' \
...
curl -s -N "127.0.0.1:$LOGS_PORT/pods/$FULL_POD_NAME/containers/$CONTAINER_NAME"
```

Note: curl's `-N` flag is necessary to disable buffering and enable real-time streaming of logs.
...
Remember to remove the `logs_server.log` file as well.

</Step>

<Step title="Delete the zone (optional)">
If you need to delete the zone and clean up all resources:

```bash
autonomy zone delete --yes
```

This will completely remove the zone, including all pods, containers, and associated resources from the Autonomy Computer.
</Step>
