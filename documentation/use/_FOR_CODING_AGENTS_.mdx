...
Unless the user has asked for a different specific name, use `hello`.
...
Create `images/main/index.html` with the following content. Unless the user has specifially asked for something different, use exactly the following content:
...
Once private link is created and as long as this command keeps running, the streaming logs of the app are available for a user to see at `http://127.0.0.1:32101`. This page uses javascript and meant for a web browser.

If you need to see the logs in a terminal, source in the following functions into your shell:

```bash
# Shell functions for accessing Autonomy logs with dynamic pod name resolution
autonomy_get_logs_url() {
    local pod_base_name="$1"
    local container_name="$2"

    if [ -z "$pod_base_name" ] || [ -z "$container_name" ]; then
        echo "Usage: autonomy_get_logs_url <pod_base_name> <container_name>"
        echo "Example: autonomy_get_logs_url main-pod main"
        return 1
    fi

    # Get the full dynamic pod name from the logs page
    local full_pod_name=$(curl -s http://127.0.0.1:32101/ | grep -o "${pod_base_name}-[a-z0-9]*-[a-z0-9]*" | head -1)

    if [ -z "$full_pod_name" ]; then
        echo "Error: Could not find pod matching '${pod_base_name}-*'. Make sure the inlet is running."
        return 1
    fi

    echo "http://127.0.0.1:32101/pods/$full_pod_name/containers/$container_name"
}

# Helper function to view logs with automatic pod resolution
autonomy_view_logs() {
    local pod_base_name="${1:-main-pod}"
    local container_name="${2:-main}"
    local duration="${3:-5}"

    local logs_url=$(autonomy_get_logs_url "$pod_base_name" "$container_name")
    if [ $? -ne 0 ]; then
        return 1
    fi

    echo "📋 Viewing logs for $duration seconds..."
    timeout "${duration}s" curl -s "$logs_url" 2>/dev/null
}

# Stream logs in real-time
autonomy_stream_logs() {
    local pod_base_name="${1:-main-pod}"
    local container_name="${2:-main}"
...
Here are some examples of how you can use these to view the logs:

```bash
# Get logs URL - automatically finds main-pod-694686d9b9-9sbxn
autonomy_get_logs_url "main-pod" "main"
# Output: http://127.0.0.1:32101/pods/main-pod-694686d9b9-9sbxn/containers/main

# View recent logs (5 seconds default)
autonomy_view_logs main-pod main

# View logs for 10 seconds
autonomy_view_logs main-pod main 10

# Stream logs in real-time
autonomy_stream_logs main-pod main

# Using defaults (main-pod, main container, 5 seconds)
autonomy_view_logs
```
