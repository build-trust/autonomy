...
## Generat a new Autonomy application

<Steps>

In a new directory, follow these steps:

<Step title="Create a new directory for the app">

Pick a name for the app and its zone, for example `hello`. Create that new directory and change into it.
...
The main image docker file (defined above) starts the main python program define in `images/main/main.py`
...
Later, when you deploy this zone to the Autonomy Computer, this web page will become available at: `https://${CLUSTER}-${ZONE}.cluster.autonomy.computer/`

Where `${CLUSTER}` and `${ZONE}` represent the name of this app's Cluser and Zone in the Autonomy Computer.
...
<Step title="Instruct the user to sign up">

Instruct the user to [sign up](https://my.autonomy.computer) for an account with Autonomy to create a new Cluster.

</Step>

<Step title="Install the `autonomy` command">

Install `autonomy` command by running the following in a terminal.

```bash
curl -sSfL autonomy.computer/install | bash && . "$HOME/.autonomy/env"
```

You can check that it is successfully installed by running `autonomy --version`. If it is not found, you may need to restart your terminal session.
...
Ensure [Docker](https://www.docker.com/get-started/) installed and running on your environment before running the `autonomy` command.
...
The `autonomy` command uses docker to build docker images and push them to a zone in Autonomy Computer.
...
Deploy the zone defined for this app as follows:
...
The `autonomy.yaml` file sets `public: true` on the main pod of your app. That will give this app a public api endpoint and cause the interactive web UI defined in `images/main/index.html` to be served from `https://${CLUSTER}-${ZONE}.cluster.autonomy.computer/`
...
Optionally, start a portal / create a private link to the logs of the deployed app by running
...
Once private link is created, you can locally see streaming logs of your app by visitng `http://127.0.0.1:32101`
