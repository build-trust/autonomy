...
# This script fetches precompiled released Autonomy binaries and installs them.
# https://github.com/build-trust/autonomy/releases

# You can call it as follow:
#
# To install the latest released version:
# curl --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/build-trust/autonomy/main/tools/install.sh | bash
#
# Or
#
# To install a specific version:
# curl --proto '=https' --tlsv1.2 -sSf https://raw.githubusercontent.com/build-trust/autonomy/main/tools/install.sh | bash -s -- --version v0.158.0
...
error() {
  ...
  echo
  echo "   If you have questions, please email us at: \`support@autonomy.computer\`"
  echo
...
display_usage() {
  echo "autonomy/install.sh - installs the autonomy binary"
  echo " "
  echo "Usage: install.sh [OPTION]..."
  echo "    -h, --help                show brief help"
  echo "    -p, --install-path PATH   specify the location for installation"
  echo "                              (default path is ~/.autonomy)"
  ...
  echo "        --no-modify-path      do not add autonomy to the PATH"
}
...
detect_binary_file_name() {
  ...
  Darwin)
    ...
    case "$_cpu_type" in
    x86_64) _file_name="autonomy-x86_64-apple-darwin" ;;
    arm64) _file_name="autonomy-aarch64-apple-darwin" ;;
    *) error "Unsupported CPU type: ${_cpu_type} on MacOS" ;;
    esac
    ;;
  Linux)
    case "$_cpu_type" in
    x86_64) _file_name="autonomy-x86_64-unknown-linux-musl" ;;
    aarch64) _file_name="autonomy-aarch64-unknown-linux-musl" ;;
    armv7l) _file_name="autonomy-armv7-unknown-linux-musleabihf" ;;
...
download() {
  ...
  if [ "$2" ]; then
    ...
    # Check if jq is available, if not use a fallback method
    if command -v jq >/dev/null 2>&1; then
      latest_tag_name=$(curl --fail --silent --show-error "https://api.github.com/repos/build-trust/autonomy/releases/latest" | jq -r '.tag_name')
    else
      # Fallback method without jq
      latest_tag_name=$(curl --fail --silent --show-error "https://api.github.com/repos/build-trust/autonomy/releases/latest" | grep '"tag_name":' | sed -E 's/.*"tag_name": *"([^"]+)".*/\1/')
    fi

    _version="$latest_tag_name"

    info "Installing latest version"
  fi

  _url="https://github.com/build-trust/autonomy/releases/download/$_version/$_binary_file_name"

  # Download to a temporary file first
  info "Downloading $_url"
  curl --proto '=https' --tlsv1.2 --location --silent --fail --show-error --output "$install_path/bin/autonomy.new" "$_url"

  info "Granting permission to execute"
  chmod u+x "$install_path/bin/autonomy.new"

  # Replace the old binary
  mv -f "$install_path/bin/autonomy.new" "$install_path/bin/autonomy"
}
...
write_env_files() {
  ...
  local _autonomy_env="$install_path/env"
  if [[ -d "$_autonomy_env" ]]; then
    error "$_autonomy_env already exists but is not a file"
    exit 1
  fi

  local _autonomy_bin="$install_path/bin"

  if [[ ! -d $_autonomy_bin ]]; then
    error "Failed to find binary directory at: $_autonomy_bin"
    exit 1
  fi

  sub_path_home "$_autonomy_bin"
  _autonomy_bin="$return_value"

  echo "#!/bin/sh
# autonomy shell setup
# affix colons on either side of \$PATH to simplify matching
case \":\${PATH}:\" in
    *:\"$_autonomy_bin\":*)
        ;;
    *)
        # Prepending path in case a system-installed autonomy needs to be overridden
        export PATH=\"$_autonomy_bin:\$PATH\"
        ;;
esac" >"$_autonomy_env"
...
add_to_path() {
  info "Adding Autonomy to PATH"
  local _autonomy_env="$install_path/env"

  sub_path_home "$_autonomy_env"
  local _autonomy_env_sub_home="$return_value"
  local _source_cmd=". \"$_autonomy_env_sub_home\""
  ...
  for rcfile in "${_rcfiles[@]}"; do
    if [[ ! -f "$_autonomy_env" ]]; then
      error "env script missing, expected script at $_autonomy_env"
      exit 1
    fi
...
main() {
  local _version="$AUTONOMY_VERSION"
  install_path="$HOME/.autonomy"
  ...
  echo
  info "Installing Autonomy Command ..."
  ...
  if [[ "$_modify_path" == "false" ]]; then
    echo "   Autonomy Command is now installed at: \"$install_path/bin/autonomy\""
    echo
    echo "   You can execute it by running:"
    echo "     \"$install_path/bin/autonomy\""
    echo
    ...
    echo "   Autonomy Command is installed at: \"$install_path/bin/autonomy\""
    echo
    echo "   To get started you may need to restart your current shell. This would"
    echo "   reload your PATH environment variable to include Autonomy's bin directory: \"$install_path/bin\""
    echo
